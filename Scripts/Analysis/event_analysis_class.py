        ...
        class EventAnalysis:
            ...
            def plot_seasonal_data(self, bins_nwvf, bins_aerosol, station_name, show_figs=True, save_figs=False, save_path=None, plot_events_pos=True, plot_events_neg=True):
                ...
                for i, season in enumerate(list(self.nwvf_data_seasonal.keys())):
                    ...
                    aerosol_stats = self.calculate_statistical_params()[0]
                    ...
                    if plot_events_neg:
                        ax[1].hist(masked_aerosol_data_neg[season],
                                ...
                                fc='lightcoral')
                    # Draw vertical lines at mean and mean +/- n_sigmas * std
                    ax[1].axvline(aerosol_stats[season]['mean'], color='k', linestyle='--', linewidth=1, label='Mean')
                    if plot_events_pos:
                        ax[1].axvline(aerosol_stats[season]['mean'] + self.n_sigmas * aerosol_stats[season]['std'], color='k', linestyle=':', linewidth=1, label=f'Mean + {self.n_sigmas} sigma')
                    if plot_events_neg:
                        ax[1].axvline(aerosol_stats[season]['mean'] - self.n_sigmas * aerosol_stats[season]['std'], color='k', linestyle=':', linewidth=1, label=f'Mean - {self.n_sigmas} sigma')
                    ...
                    # Add the figure and axes to the lists
                ...
                if save_figs and save_path:
                    for i, fig in enumerate(self.figures):
                        fig.savefig(f'{save_path}/{station_name}_{i+1}.png')
            ...
            def plot_all_data(self, station_name, bins_nwvf=32, bins_aerosol=32, show_figs=True, save_figs=False, save_path=None, plot_events_pos=True, plot_events_neg=True):
                '''
                    Plot all the data in histograms - with and without masking.

                    Parameters:
                    -----------
                    station_name : str
                        Name of the station.
                    bins_nwvf : int or dict, optional
                        Number of bins for the nwvf data histogram or dictionary of list of bins for each season.
                        Default is 32.
                    bins_aerosol : int or dict, optional
                        Number of bins for the aerosol data histogram or dictionary of list of bins for each season.
                        Default is 32.
                    ...
                    save_path : str, optional
                        Path to save the figures. Required if save_figs is True.
                    plot_events_pos : bool, optional
                        Whether to plot events above the threshold. Default is True.
                    plot_events_neg : bool, optional
                        Whether to plot events below the threshold. Default is True.
                '''

                # Check if bins_nwvf and bins_aerosol are integers
                if isinstance(bins_nwvf, int):
                    bins_nwvf = {season: bins_nwvf for season in self.seasons}
                if isinstance(bins_aerosol, int):
                    bins_aerosol = {season: bins_aerosol for season in self.seasons}

                # Plot the seasonal data
                self.plot_seasonal_data(bins_nwvf, bins_aerosol, station_name, show_figs, save_figs, save_path, plot_events_pos, plot_events_neg)
